  ...
  return (
    <div className={`admin ${folded ? "admin--folded" : ""}`}>
      ...
      <main className="admin__main">
        ...
        {/* 담당자 계정 관리 — 마스터 + 마스터가 임명한 담당자 관리자 (2026-09-03).
            직접 등록·구분 이동·관리자 임명은 Rules상 마스터만 가능하므로 마스터에게만 노출한다. */}
        {section === "accounts" && canAccess(session.role, "accounts") && (
          <Accounts
            title="담당자 계정 관리"
            description="담당자(행정) 계정의 가입 신청을 승인하거나, 사용을 중지·삭제합니다. 상담사 계정은 상담사 워크스페이스에서 별도 관리합니다."
            roles={["ADMIN_LEAD", "ADMIN"]} // 마스터 계정은 어떤 목록에도 노출하지 않음 (§6.4)
            canPromote={session.role === "MASTER"}
            canCreate={session.role === "MASTER"}
            reassignTo={session.role === "MASTER" ? "COUNSELOR" : undefined}
          />
        )}
      </main>
